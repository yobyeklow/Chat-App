...
CREATE TABLE IF NOT EXISTS users(
    ...
    user_password VARCHAR(150) NOT NULL UNIQUE,
    user_role INT NOT NULL DEFAULT 1 CHECK(user_role IN (1,2)),
    user_status INT NOT NULL DEFAULT 1 CHECK(user_role IN (1,2,3)),
    user_created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
    user_updated_at TIMESTAMPTZ NOT NULL DEFAULT now(),
    user_deleted_at TIMESTAMPTZ DEFAULT NULL
);
-- Comments
COMMENT ON COLUMN users.user_role IS 'User Role: 1 - User, 2 - Adminstator';
...
-- Create Trigger
CREATE OR REPLACE FUNCTION update_user_update_at()
RETURNS TRIGGER AS $$
BEGIN
